...
export default ProjectList = ({ project }) => {
    return (
        <View style={{
            minHeight: 60, borderBottomWidth: 1,
            borderBottomColor: '#D1D1D1', flexDirection: 'row'
        }}>
            <View style={styles.box}>
                <Text text={project.clientName} color={themeColor} bold={true} font={20} />
                <Text text={"Address line 1 , line 2 , line 2"} color={themeColor} font={14} />
            </View>
            <View style={styles.box}>
                <Text text={project.projectNumber} color={themeColor} font={14} bold={true} />
            </View>
            <View style={[styles.box, { alignItems: 'center' }]}>
                <Button buttonStyle={{
                    backgroundColor: "#fff", borderColor: project.status === 'Sent' ? "green" : "red",
                    borderWidth: 1, borderRadius: 25
                }}
                    height={35}
                    width={100}
                    textStyle={{ color: project.status === 'Sent' ? "green" : "red", fontSize: 14 }}
